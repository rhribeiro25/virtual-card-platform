...
public class ProviderWriter extends AbstractBatchWriter<Provider, String> {
    ...
    public ProviderWriter(BatchAuditImportMongoTemplate batchAuditImportMongoTemplate, PersistenceUtils persistenceUtils, ProviderUsecase providerUsecase) {
    ...
    protected void setAuditStatus(BatchAuditImport item, Boolean exists) {
        if (exists == null) item.setStatus(PROVIDER_NO_ACTION);
        else if (exists) item.setStatus(PROVIDER_UPDATED);
        else item.setStatus(PROVIDER_PERSISTED);
    }
